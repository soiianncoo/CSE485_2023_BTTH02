...
<body>
    ...
    <main class="container mt-5 mb-5">
        <?php
            ...
            $sql = "SELECT * from baiviet where ma_bviet = ".$_GET['id'];
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_array($result);
            if(!empty($_GET['action']) && $_GET['action'] == "update"){
                if(!empty($_POST['txtCatName']) && isset($_POST['txtCatName'])){
                    $update = mysqli_query($conn,"UPDATE `baiviet` set `tieude`='".$_POST['txtCatName']."' where `ma_bviet`=".$_POST['txtCatId']."");
                    if(!empty($update)){?>
                        <p>Sửa Bài viết thành công</p><a href="./article.php">Quay lai trang bài viết</a>
                    <?php exit;}else{
                        echo 'Lỗi!';exit;
                    }
                }
